...
func TestLogin_shouldNotFound(t *testing.T) {
	...
	users := sqlmock.NewRows([]string{"user_id", "user_name", "email", "password", "role", "deposit"})
...
func TestLogin_shouldFound(t *testing.T) {
	...
	users := sqlmock.NewRows([]string{"user_id", "user_name", "email", "password", "role", "deposit"}).
		AddRow(12, "", "yoland", "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJlbWFpbCI6InlvbGFuZCIsImV4cCI6MTcxODQ2Mzk3OSwiaWQiOjEyLCJyb2xlIjoidXNlciJ9.nY6_jG_hh_37jGfcfG8pbyckPwNmhinYqLL7rGabyYY", "user", 0.0)
...
func TestTopUp(t *testing.T) {
	...
	users := sqlmock.NewRows([]string{"user_id", "user_name", "email", "password", "role", "deposit"}).
		AddRow(12, "", "yoland", "eabyYY", "user", 100)
